...
#
# Usage:
#   ./publish-update.sh                # full: build + manifest (dry-run)
#   DSH_LIGHT=1 ./publish-update.sh    # light: build + manifest (dry-run)
#   ./publish-update.sh --both         # both variants, one release (dry-run)
#   ./publish-update.sh --release      # also create the GitHub release + upload
#   ./publish-update.sh --release --both
#   DSH_RELEASE_NOTES="…" ./publish-update.sh --release
#
...
build_and_manifest() {
  ...
  (cd "$ROOT" && DSH_LIGHT="$LIGHT_FLAG" ./build.sh)

  echo "==> Packaging DMG ($APP_NAME)"
  (cd "$ROOT" && APP_NAME="$APP_NAME" ./make-dmg.sh)
...
if [ "$DO_RELEASE" = "1" ]; then
    ...
    echo "     ./publish-update.sh --release [$([ "$DO_BOTH" = "1" ] && echo "--both")]"
fi
