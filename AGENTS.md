...
- `scripts/**`、`.github/**`（CI）、顶层 `*.sh`（build / make-dmg / install /
  publish-update / dev-update）、`gen-icon.swift`。
...
- **构建前置询问**：`build.sh` / `make-dmg.sh` / `dev-update.sh` /
  `publish-update.sh` 等构建/打包耗时长（通常 1–2 分钟以上），**动手前必须先
  询问用户**，得到确认后再用后台任务运行；不要擅自触发构建。
