...
final class UpdateManager: NSObject, URLSessionDownloadDelegate {
    ...
    @objc func restartToInstall(_ sender: Any?) {
        ...
        alert.messageText = "Restart to Install \(Self.currentProductName) \(revLabel(version: pending.version, build: pending.build))"
    ...
    private func handleManifest(data: Data?, error: Error?, interactive: Bool) {
        ...
        guard isNewer(manifest) else {
            if interactive {
                presentOK(title: "Check for Updates",
                          message: "You're up to date — \(Self.currentProductName) \(revLabel(version: currentVersion, build: currentBuild)).")
            }
            return
        }
    ...
    private func offerDownload(_ manifest: UpdateManifest) {
        let alert = NSAlert()
        alert.alertStyle = .informational
        alert.messageText = "\(Self.currentProductName) \(revLabel(version: manifest.version, build: manifest.build)) is available"
    ...
    private func startDownload(_ manifest: UpdateManifest) {
        ...
        if url.isFileURL {
            ...
            showProgress(title: "Preparing \(Self.currentProductName) \(manifest.version)…", indeterminate: true)
            ...
            showProgress(title: "Downloading \(Self.currentProductName) \(manifest.version)…")
    ...
    private func showProgress(title: String, indeterminate: Bool = false) {
        ...
        win.center()
        // Never attach as a sheet: beginSheet here has repeatedly raised an
        // NSException (SIGABRT) via _beginWindowBlockingModalSessionForSheet:
        // even when the key window is visible, AppKit's modal-session machinery
        // is fragile (invisible/minimized/non-active-space/modal conflicts all
        // throw). Always present the progress as a standalone window — no modal
        // session, nothing to throw (spec S-0001 FR-9.13 / T-6).
