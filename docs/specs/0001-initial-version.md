...
- **FR-9.9** 清单 URL 与 `dmgUrl` 必须（MUST）支持 `file://`（本地开发）：本地
  构建经 `scripts/dev-update.sh` 生成清单与 DMG 后，走与 GitHub 发布完全相同的"后台暂存
  → 提醒 → 点按更新"流程。
...
- **FR-9.12** 本地开发：`scripts/dev-update.sh` 在生成 `file://` 清单后必须（MUST）
  **自动**将运行中目标 bundle 的 `DSHUpdateManifestURL` 覆盖设置为该本地清单
  （`com.deepseek.dsh.desktop` / `.light` 按变体，见 §8.3），无需用户手动执行
  `defaults write`；该覆盖在本地更新安装成功后由 `dsh-updater` 自动清除、回归
  默认清单（FR-9.11）。
- **FR-9.13** 更新进度 UI 必须（MUST）以**独立窗口**呈现，**不得（MUST NOT）以
  sheet / 模态会话**形式挂到窗口上：AppKit 的 `beginSheet`（经
  `_beginWindowBlockingModalSessionForSheet:`）在多种窗口状态下（不可见、最小化、
  非活动空间、模态会话冲突等）都会抛 NSException 导致 SIGABRT，已在真实运行中
  对**可见**窗口反复复现（见 §8.6 T-6）；进度统一经 `makeKeyAndOrderFront` 用
  独立窗口展示，从根上规避模态崩溃。
- **FR-9.14** 更新相关**用户可见文案**必须（MUST）使用当前变体名称
  （`currentProductName`：full → `dsh-desktop`，light → `dsh-desktop-light`），
  不得（MUST NOT）硬编码为 `dsh-desktop`——否则 light 变体的「有可用更新 / 已是
  最新 / 重启并安装 / 下载 / 准备」等弹窗与进度文案会显示错误的变体名。
...
`dmgUrl` 及清单 URL 本身均可为 `file://`（本地开发流程，见 §8.3 `scripts/dev-update.sh`）；
...
- **次级比较贯穿整个管线**：`isNewer`、`restorePendingIfAny`（重启后恢复已暂存
  ...
  - "跳过此版本"以 `version`+`build` 为键，跳过某一 build 不得（MUST NOT）屏蔽
    ...
    如 `0.1.0.0`）在首次遇到**同版本**携带 `build` 的清单时被丢弃——它无法表达
    "被跳过的具体构建"，不得（MUST NOT）作为整个版本串的通配符生效，否则会吞掉
    同版本串的后续新 build；仅在清单本身同样**不携带** `build`（旧格式清单）时，
    旧纯版本值才按原"跳过此版本"语义生效（向后兼容）。跳过其他版本的旧值保留，
    仅在其自身版本遇到携带 `build` 的清单时才被丢弃。
...
### 8.3 发布（`scripts/publish-update.sh` / `scripts/dev-update.sh`）

- `./scripts/publish-update.sh` —— 构建 + 打包 + 生成 `dist/update-manifest.json`。
...
- `./scripts/publish-update.sh --release` —— 额外创建 GitHub release（tag
  `v<VERSION>`）并通过 `gh` 或 `GITHUB_TOKEN` 上传 DMG 与清单。
- `./scripts/dev-update.sh` —— 本地开发：构建 + 打包 + 生成 `dist/dev-update.json`
  （`file://` 清单），并在生成后**自动**将运行中目标 bundle 的
  `DSHUpdateManifestURL` 覆盖设置为该本地清单（FR-9.12），无需手动执行
  `defaults write`，让运行中的应用直接走与 GitHub 发布完全相同的更新流程
  （FR-9.9）。`DSH_LIGHT=1` 时构建 light 变体、生成 `dist/dev-update-light.json`
  （`app: dsh-desktop-light`）并指向 `com.deepseek.dsh.desktop.light`，实现 light
  的**独立本地升级**。本地更新安装成功后，覆盖由 `dsh-updater` 自动清除、回归
  默认（GitHub）清单（FR-9.11）。桌面修订由 scripts/build.sh 从 git 历史**最近
...
- 生成清单时对所有字符串字段（`version` / `build` / `app` / `platform` /
  `dmgUrl` / `dmgSha256` / `releaseNotes`）做 **JSON 转义**（RFC 8259，经
  `python3` 的 `json.dumps(..., ensure_ascii=False)`，即 `json_escape()`），保证
  `DSH_RELEASE_NOTES` 含换行 / 引号 / 反斜杠等字符时产物仍是合法 JSON，客户端
  `JSONSerialization` 可直接解析（`scripts/publish-update.sh` 与
  `scripts/dev-update.sh` 均适用）。
...
- 版本串相同（`DSH_DESKTOP_REV` 相同）时，`build`（git 短 revision）只要与当前
  不同即视为**新 build**——因此本地开发（`scripts/dev-update.sh`）每次构建的 revision
  变化即可被检测为更新，无需改动可见版本（§8.3、FR-9.9）。

### 8.5 Light 变体（`dsh-desktop-light`）

`DSH_LIGHT=1 ./scripts/build.sh` 构建一个**代码完全一致**的轻量变体
...
- 打包：`DSH_LIGHT=1 ./scripts/make-dmg.sh`（或 `APP_NAME=dsh-desktop-light`）产出
  `dsh-desktop-light-<VERSION>-arm64.dmg`（§8.2）。
- **可独立升级**：清单含 `app: dsh-desktop-light`，默认清单 URL 为
  `update-manifest-light.json`（§7.4）；本地开发用 `DSH_LIGHT=1 ./scripts/dev-update.sh`
...
- **T-6（进度 UI 崩溃回归）**：更新进度必须（MUST）用**独立窗口**呈现，禁止
  （MUST NOT）对任意窗口调用 `beginSheet`（模态会话对不可见/最小化/非活动空间
  窗口会抛 NSException 导致 SIGABRT，真实运行已对可见窗口复现，FR-9.13）。人工
  验证：窗口隐藏/最小化/切换空间/应用后台状态下触发更新均不得崩溃，且进度可见。

### 8.7 仓库布局（`scripts/` 归置与 `gen-icon.swift` 移除）

顶层构建/发布脚本统一归置到 `scripts/`（**原 S-0002 决策已并入本节**，原
`S-0002-FR-1..7` / `S-0002-NFR-1..2` 由下列要求覆盖）：

- **脚本归置**：`build.sh` / `make-dmg.sh` / `install.sh` / `publish-update.sh` /
  `dev-update.sh` 5 个 shell 脚本必须（MUST）位于 `scripts/`，仓库根目录不得
  （MUST NOT）保留同名脚本（原 S-0002-FR-1）；停用的程序化图标生成器
  `gen-icon.swift` 已删除，仓库内不得（MUST NOT）残留其引用（原 S-0002-FR-6）。
- **根路径解析**：各脚本以 `ROOT="$(cd "$(dirname "$0")/.." && pwd)"` 解析仓库根，
  `$ROOT/dist/...`、`$ROOT/build/...` 相对引用不得（MUST NOT）失效
...
- **行为不变**：本次整理为纯路径调整，不得（MUST NOT）改变脚本的构建/发布行为
  （原 S-0002-NFR-1）；各脚本必须（MUST）通过 `bash -n` 语法校验
  （原 S-0002-NFR-2）。
...
| 跳过的版本 | `UserDefaults` `DSHSkippedUpdateVersion`（`version@build` 复合键） | 以 `version`+`build` 为键；同版本串的新 build 不受旧跳过影响，在新版本出现前一直生效。旧版遗留的裸版本值（不含 `@`）在遇到**同版本**携带 `build` 的清单时被丢弃，仅对不携带 `build` 的旧格式清单按原"跳过此版本"语义生效 |
...
- 内嵌后端版本固定在 `@deepseek-ai/dsh@0.1.0-rc.6`，直至更新 `scripts/build.sh`。
