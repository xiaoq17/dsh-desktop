...
- 改动 `src/`、`assets/`、`scripts/`、`.github/`、顶层 `*.sh` 等（影响行为/
...
3. **Build**:
   ```bash
   ./build.sh      # compiles Swift, assembles dist/dsh-desktop.app
   ./make-dmg.sh   # packages dist/dsh-desktop-<version>-arm64.dmg
   ```
   `build.sh` downloads the official Node runtime and installs
...
- Run the checks that CI runs locally:
  ```bash
  bash -n build.sh make-dmg.sh install.sh publish-update.sh   # shell syntax
  ```
