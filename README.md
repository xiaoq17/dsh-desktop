...
cd dsh-desktop
./install.sh          # build → /Applications → launch
./install.sh --dmg    # install from a pre-built .dmg
```
...
DSH_RELEASE_NOTES="• What's new in 0.1.0.0" ./publish-update.sh --release
```
...
[Version scheme](#size--notes)), then `publish-update.sh --release`.
...
```bash
DSH_DESKTOP_REV=1 ./dev-update.sh          # build + DMG + file:// manifest
defaults write com.deepseek.dsh.desktop DSHUpdateManifestURL \
  "file://$(pwd)/dist/dev-update.json"     # point the running app at it
```
...
| Knob | How to set |
|---|---|
| Manifest URL (build-time) | `DSH_UPDATE_MANIFEST_URL="https://…" ./build.sh` (default: this repo's `releases/latest`) |
| Manifest URL (per machine) | `defaults write com.deepseek.dsh.desktop DSHUpdateManifestURL "https://…"` (`file://…` for local dev) |
| GitHub repo (publish) | `DSH_GITHUB_REPO="owner/repo" ./publish-update.sh --release` |
| Release notes | `DSH_RELEASE_NOTES="…" ./publish-update.sh --release` |
...
```bash
./build.sh          # download Node + install dsh deps + compile → dist/*.app
./gen-icon.swift    # regenerate build/AppIcon.icns
...
- Target: macOS 13.0+, Apple Silicon (arm64). Rebuild for x86_64 by changing
  `-target` in `build.sh` to `x86_64-apple-macos13.0` and the Node tarball URL.
